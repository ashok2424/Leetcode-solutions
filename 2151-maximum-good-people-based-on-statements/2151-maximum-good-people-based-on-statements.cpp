...
public:
   int ans  = 0; vector<vector<int>> a;
    void subsets(int n, int i, string &temp,int good){
        if(i==n){
            if(is_valid(temp,a)) ans = max(ans,good);
            return;
        }
        temp.push_back('1');
        subsets(n, i+1, temp,good+1);
        temp.pop_back();
        
        temp.push_back('0');  
        subsets(n, i+1, temp,good);
        temp.pop_back();
    }
    
    int maximumGood(vector<vector<int>>& nums) {
        string temp = ""; a = nums;//using temp as vector<int> gives TLE 
        subsets(a.size(), 0,temp,0);  return ans;
    }
